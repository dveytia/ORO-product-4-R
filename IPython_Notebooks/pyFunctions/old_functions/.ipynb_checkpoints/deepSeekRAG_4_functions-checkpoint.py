...
def process_pdf(file):
    try:
        ...
        if vector_store is None:
            ...
            nonMethodsText = extract_text_excluding_sections(pdf_path)
            if nonMethodsText:
                chunked = split_text_unstructured([nonMethodsText])
            else:
        ...
        if len(interventions)> 0:
            for i, interventionResponse in enumerate(interventions):
                ...
                for item in variableDescriptions:
                    ...
                    if variable == 'study method' and vector_store_methods:
