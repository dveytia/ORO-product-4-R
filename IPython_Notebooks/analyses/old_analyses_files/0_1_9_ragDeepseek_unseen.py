...
This applies v9 to all unseen articles
"""
...
pdfs_directory = f'{dataFolder}/data/raw-data/full-texts/'
faiss_directory = f'{dataFolder}/data/derived-data/testList_vectorstores_partitioned_bgem3_300/'
answer_directory = f'{dataFolder}/outputs/unseenDeepseekAnswers_v{v}/'
...
for file in tqdm(answered):

    file_path = f"{answer_directory}{file}_code.txt"
    with(open(file_path)) as f:
        results_dict = json.load(f)

    doc_id = results_dict.get('id')
    responses = results_dict.get('coding response',[])
    read_error = results_dict.get('pdf reading error')

    if responses and not read_error:
        response = responses[0][0]
        if isinstance(response, dict) and np.isnan(response.get('include')):
            ...
            responses[0][0] = answer_dict
            codeResults = responses[0]

            result = {'id': doc_id, 'coding response': [codeResults]}
...
"""
compiled_answer_directory = '/homedata/dveytia/Product_4_data/outputs/unseenDeepseekAnswers_compiled/'
os.makedirs(compiled_answer_directory, exist_ok=True)
...
for file in answered:
    ...
    for response in responses[0]:
        ...
        ## No need to recover parsing errors because this is now done as part of codeVariable
...
answers_df = answers_df.drop(['study results source text','study results error_message','study_results','study results raw output'], axis=1)
...
## Save 
answers_df.to_excel(f'{compiled_answer_directory}unseen_answers_df_v{v}.xlsx', index=False)

# save without test list ids
human_testList_df = pd.read_excel(f'/homedata/dveytia/Product_4_data/data/raw-data/coding-anwers-human/testList_answers_df_v{v}_human.xlsx')
testListIds = list(set(human_testList_df.id))

answers_df.loc[~answers_df.id.isin(testListIds).to_numpy(),:].sort_values(by=['id']).to_excel(f'{compiled_answer_directory}unseen_answers_df_noTestList_v{v}.xlsx', index=False)
...
if len(answers_study_results)>0:
    ...
    ## Save
    answers_study_results_df = answers_study_results_df.sort_values(by=['id'])
    
    answers_study_results_df.to_excel(
    f'{compiled_answer_directory}unseen_answers_df_studyResults_v{v}.xlsx', index=False)
    
    answers_study_results_df.loc[~answers_study_results_df.id.isin(testListIds).to_numpy(),:].sort_values(by=['id']).to_excel(f'{compiled_answer_directory}unseen_answers_df_studyResults_noTestList_v{v}.xlsx', index=False)
