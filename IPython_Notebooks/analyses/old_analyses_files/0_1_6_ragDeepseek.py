...
Previous version
- study_result seemed to perform well
- all other variables had coding errors -- I think they were too complicated?
- check why country names had parsing error for "country_names": ["Uganda", "Kenya", "Nigeria"]
- maybe resort to product 2 method -- the union seems to be causing trouble...

Modifications
- reduce chunk size and increase faissK
- fixed over-writing error for source text
- expand out source text in results so I can tag them if i want
- code oro types individually
- increase to 14b model? Or maybe try phi4?
...
with open(f'/home/dveytia/IPython_Notebooks/Product_4/pyFunctions/deepSeekRAG_6_functions.py') as f:
    exec(f.read())
