...
This version updates was set up to list oro type(s) as a free string. It will also code variables for all articles with a valid intervention
...
for file in answered:
    ...
    if answer_dict.get('interventions',[]):
        ...
        tempDatMelt = tempDatMelt.drop(columns=['coded variables'])

        if 'error_message' in tempDatMelt.columns:
            tempDatMelt = tempDatMelt.drop(columns=['error_message'])
...
ids = answers_df.pop('id') 
answers_df.insert(0, 'id', ids) 
